...
app.config['SECRET_KEY'] = 'sem7project'  
app.config['SESSION_TYPE'] = 'filesystem'

app.config['MYSQL_HOST'] = 'localhost'
app.config['MYSQL_USER'] = 'root'
app.config['MYSQL_PORT'] = 3390
...
app.config['MAIL_USERNAME'] = 'mahekchougle2002@gmail.com'
app.config['MAIL_PASSWORD'] = 'vexm daqi vuwi ursn'
...
sender = 'mahekchougle3@gmail.com'
...
def contact():
	if request.method == 'POST':
		careEmail = "mahekchougle3@gmail.com"
...
def lostpassword():
	if request.method == 'POST':
		...
		if results > 0:
			...
			msg1 = Message('ProctoXpert - OTP Verification for Lost Password', sender = sender, recipients = [lpemail])
...
def register():
	if request.method == 'POST':
		...
		print("Generated OTP:", sesOTP) 
		print("Session Data in register route:", session)
	
	
		mOTP = session.get('tempOTP')
		print(f"Session OTP: {mOTP}")

		msg1 = Message('ProctoXpert.ai - OTP Verification', sender = sender, recipients = [email])
		msg1.body = "New Account opening - Your OTP Verfication code is "+sesOTP+"."
		mail.send(msg1)

		

		if 'tempOTP' in session:
			return redirect(url_for('verifyEmail'))
		else:
			flash("Failed to set OTP in the session.")
			return redirect(url_for('register')) 
...
def verifyEmail():
	if request.method == 'POST':
		...
		print(f"theOTP: {theOTP}, session['tempOTP']: {mOTP}")
		print("Session Data in register route:", session)
...
def share_details_emails():
	if request.method == 'POST':
		...
		msg1 = Message('EXAM DETAILS - ProctoXpert.ai', sender = sender, recipients = [emailssharelist])
